...
class VGGmodel(object):
    ...
    def __init__(self):
        ...
        data = sio.loadmat(os.path.join(os.path.dirname(__file__), 'caltech_image.mat'))
        self.categories = data['categories']
        self.num_classed = len(self.categories)

    def load_model(self):
        # load json and create model
        json_file = open(os.path.join(os.path.dirname(__file__), 'caltech/model.json'), 'r')
        loaded_model_json = json_file.read()
        json_file.close()
        self.model = model_from_json(loaded_model_json)
        # load weights into new model
        self.model.load_weights(os.path.join(os.path.dirname(__file__), "caltech/model.h5"))
    ...
    def test(self):
        data = sio.loadmat(os.path.join(os.path.dirname(__file__), 'caltech_del.mat'))
        X_test = data['X_test']
        y_test = data['y_test']
...
class CIFAR100model(object):
    ...
    def load_model(self):
        # load json and create model
        json_file = open(os.path.join(os.path.dirname(__file__), 'model/cifar100/100model_70.json'), 'r')
        loaded_model_json = json_file.read()
        json_file.close()
        self.model = model_from_json(loaded_model_json)
        # load weights into new model
        self.model.load_weights(os.path.join(os.path.dirname(__file__), "model/cifar100/100model_70.h5"))
        print("Loaded model from disk")
